...
# $Header: /var/cvsroot/gentoo-x86/net-wireless/wpa_supplicant/wpa_supplicant-0.5.8.ebuild,v 1.4 2007/09/25 12:11:43 uberlord Exp $
...
src_unpack() {
	...
	if use dbus ; then
		epatch "${FILESDIR}"/${PN}-0.5.7-dbus-blobs.patch
		epatch "${FILESDIR}"/${PN}-0.5.7-dbus-iface-segfault-fix.patch
		epatch "${FILESDIR}"/${PN}-0.5.7-dbus-permissions-fix.patch
	fi
...
src_install() {
	...
	if use dbus ; then
		...
		# Due to being in an if block, need to quote the name of the file as
		# well.
